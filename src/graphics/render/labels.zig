...
pub fn drawLabels(canvas: *gpu.Canvas, elapsed: f64) void {
    ...
    if (state.mode_mix < 0.16) {
        text.draw(canvas, title, L.art_x_expanded, L.card_y + L.mini_title_y, L.mini_text_width, 15.0 * scale, 1.0, true, primary_color, state.marquee_offset, false, false);
        text.draw(canvas, artist, L.art_x_expanded, L.card_y + L.mini_artist_y, L.mini_text_width, 11.0 * scale, 1.0, false, secondary_color, 0.0, false, true);
        return;
    }
    ...
    text.draw(canvas, e_text, x, state.layout.bar_y + 11.0, 100.0, 12.0 * scale, 1.0, false, secondary_color, 0.0, false, false);

    const d_text = std.fmt.bufPrint(&buffer, "{d}:{d:0>2}", .{ d / 60, d % 60 }) catch return;
    text.draw(canvas, d_text, x, state.layout.bar_y + 11.0, state.layout.bar_w, 12.0 * scale, 1.0, false, secondary_color, 0.0, true, false);
}
