...
pub fn drawLabels(canvas: *gpu.Canvas, elapsed: f64) void {
    ...
    text.draw(canvas, title, x, lerp(L.card_y + L.mini_title_y, state.layout.art_y + 8.0, t), width, 17.0, lerp(15.0, 17.0, t) / 17.0, true, primary_color, 0.0, false, true);
    text.draw(canvas, if (artist.len > 0) artist else "Play something to get started", x, lerp(L.card_y + L.mini_artist_y, state.layout.art_y + 32.0, t), width, 14.0, lerp(11.0, 14.0, t) / 14.0, false, secondary_color, 0.0, false, true);
    ...
    text.draw(canvas, e_text, x, state.layout.bar_y + 13.0, 100.0, 12.0, 1.0, false, secondary_color, 0.0, false, false);

    const d_text = std.fmt.bufPrint(&buffer, "{d}:{d:0>2}", .{ d / 60, d % 60 }) catch return;
    text.draw(canvas, d_text, x, state.layout.bar_y + 13.0, state.layout.bar_w, 12.0, 1.0, false, secondary_color, 0.0, true, false);
}
