...
pub export fn widget_text_clipped(pixels: [*]u32, width: usize, height: usize, utf8: [*]const u8, length: usize, x: f64, y: f64, clip_x: f64, clip_width: f64, font_size: f64, bold: c_int, r: u8, g: u8, b: u8) callconv(.c) void {
    ...
    defer macos.CGContextRelease(context);
    var ascent: f64 = 0;
    _ = macos.CTLineGetTypographicBounds(line, &ascent, null, null);
    macos.CGContextClipToRect(context, macos.rect(clip_x, 0, clip_width, @floatFromInt(height)));
    macos.CGContextSetShouldAntialias(context, true);
    macos.CGContextSetTextPosition(context, x, @as(f64, @floatFromInt(height)) - y - ascent);
    macos.CTLineDraw(line, context);
}
