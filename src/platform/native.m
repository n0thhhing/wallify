...
void wallify_profile_scene(double seconds) {
    ...
    atomic_fetch_add(
        &sceneNanos,
        (unsigned long)(seconds * 1e9)
    );

    unsigned long frames =
        atomic_fetch_add(&sceneFrames, 1) + 1;

    if (frames % 300 == 0) {
        unsigned long rendered =
            atomic_load(&renderedFrames);

        fprintf(
            stderr,
            "Wallify profile: frames=%lu scene_cpu_ms=%.3f gpu_ms=%.3f commands_per_frame=%.1f asset_upload_bytes=%lu full_frame_upload_bytes=0\n",
            frames,
            atomic_load(&sceneNanos) / (double)frames / 1e6,
            rendered
                ? atomic_load(&gpuNanos) / (double)rendered / 1e6
                : 0,
            rendered
                ? atomic_load(&drawCalls) / (double)rendered
                : 0,
...
+ (instancetype)sharedTarget {
    ...
    dispatch_once(&onceToken, ^{
        target =
            [[WallifyStatusMenuTarget alloc] init];
    });
...
- (void)statusOpenSpotify:(id)sender {
    ...
    [[NSWorkspace sharedWorkspace]
        openURL:[NSURL URLWithString:@"spotify:"]];
}
...
static void movePanel(
    ...
    NSRect screen =
        (panel.screen ?: NSScreen.mainScreen).visibleFrame;
...
bool wallify_create(
    ...
    if (!device || !queue) {
        NSLog(
            @"Wallify: Metal device or command queue unavailable."
        );
    ...
    if (!defaultLibrary) {
        NSLog(
            @"Wallify: Failed to load Metal shaders: %@",
            error
        );
    }
    ...
    if (defaultLibrary) {
        ...
        pipelineStateDescriptor.label =
            @"Wallify GPU compositor";

        pipelineStateDescriptor.vertexFunction =
            [defaultLibrary
                newFunctionWithName:@"vertex_main"];
        ...
        if (!pipelineState) {
            NSLog(
                @"Wallify: Failed to create pipeline state: %@",
                error
            );
        }
    }
    ...
    surface.presentsWithTransaction =
        YES;

    surface.opaque =
        NO;

    surface.contentsScale =
    ...
    NSMenu *menu =
        [[NSMenu alloc]
            initWithTitle:@"Wallify"];

    WallifyAppDelegate *delegate =
        [[WallifyAppDelegate alloc] init];
    ...
    settings.target =
        [WallifyStatusMenuTarget sharedTarget];
...
static void presentLatest(void) {
    @autoreleasepool {
        ...
        id<MTLTexture>
            textures[WALLIFY_MAX_TEXTURES];

        for (
            size_t i = 0;
            i < WALLIFY_MAX_TEXTURES;
            i++
        ) {
            textures[i] =
                latestTextures[i];
...
void wallify_swap_textures(
    int src,
    int dest
) {
    if (
        src < 0 ||
        src >= WALLIFY_MAX_TEXTURES ||
        dest < 0 ||
        dest >= WALLIFY_MAX_TEXTURES
    ) {
        return;
    }
...
void wallify_load_texture(
    ...
    if (
        textureID < 0 ||
        textureID >= WALLIFY_MAX_TEXTURES ||
        !width ||
        !height
    ) {
        return;
...
float wallify_glow_extent(
    float artSize
) {
    return ceilf(
        artSize *
        fmaxf(
            WALLIFY_GLOW_SCALE_X,
            WALLIFY_GLOW_SCALE_Y
        ) +
        6 *
        WALLIFY_GLOW_BLUR
    );
...
void wallify_prepare(void) {
    @autoreleasepool {
        ...
        if (
            [bundle.bundlePath.pathExtension
                isEqualToString:@"app"]
        ) {
            NSFileManager *fm =
...
bool wallify_panel_offsets(
    ...
    double primTop =
        primFrame.origin.y +
        primFrame.size.height;

    double screenVisibleTop =
        visible.origin.y +
        visible.size.height;
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (@available(macOS 26.0, *)) {
            if (active) {
                ...
                if (!NSEqualRects(globalGlassView.frame, frame)) globalGlassView.frame = frame;
                ...
                globalMetalView.frame = NSMakeRect(-x, -flippedY,
                    atomic_load(&surfaceWidth), atomic_load(&surfaceHeight));
                [CATransaction commit];
                return;
            }
        }
...
extern void wallify_media_key_event(
    int keyCode
);

static CFMachPortRef mediaKeyTap =
    NULL;

static CFRunLoopSourceRef mediaKeyTapSource =
    NULL;

static int mediaKeyTarget =
    0;
...
static CGEventRef mediaKeyCallback(
...
void wallify_update_media_key_tap(
    int target
) {
    dispatch_async(
        dispatch_get_main_queue(),
        ^{
            ...
            if (
                ...
                NSLog(
                    @"Wallify: Accessibility permission required for media key interception."
                );
            ...
            if (!mediaKeyTap) {
                NSLog(
                    @"Wallify: Failed to create CGEventTap (check Accessibility permission)."
                );
            ...
            NSLog(
                @"Wallify: Media key tap installed (target=%d).",
                target
            );
        }
    );
